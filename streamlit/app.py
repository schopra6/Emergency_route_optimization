...
if "traffic_area" not in st.session_state:
    st.session_state.traffic_area = None
...
with st.sidebar:
    ...
    NUM_Ambulances = st.number_input("Number of Ambulances", min_value=1, value=15)
    AMBULANCE_CAPACITY = st.number_input("Bus Capacity", min_value=1, value=80)
...
if IS_DATA_LOADED:
    ...
    with col2:
        if st.session_state["depot_location"] is not None:
            ...
            with st.expander("**Add a disaster area**"):
                add_traffic_area = st.button(
                    "Add disaster area",
                    use_container_width=True,
                    key="add_traffic_area",
                    type="primary",
                )

                m = maps.add_traffic_area(random_stops_df_gpd)

                traffic_area = st_folium(
                    m,
                    key="traffic_map",
                    width=1000,
                    height=500,
                ...
                if add_traffic_area:
                    if traffic_area.get("all_drawings") is not None:
                        st.session_state["traffic_area"] = traffic_area[
                            "all_drawings"
                        ][0]["geometry"]["coordinates"]

    if st.session_state["traffic_area"] is not None:
        traffic_bounds = st.session_state["traffic_area"][0]

        stops_in_traffic_area, traffic_area = load.add_traffic_area(
            random_stops_df_gpd, [*traffic_bounds[0], *traffic_bounds[2]]
        )

        with st.expander("**Show unsolved network**"):
            st.markdown(f"`Number of stops in sample: {len(random_stops_df_gpd)}`")
            st.markdown(
                f"`Number of stops in disaster area: {len(stops_in_traffic_area)}`"
            )
    ...
    if (
        ...
        and st.session_state["traffic_area"] is not None
    ):
        ...
        if st.session_state["model_solved"]:
            ...
            with st.expander("**Show solved network**"):
                ...
                st.markdown(
                    f"`Number of Ambulances used: {model_['ambulance_path_df'].ambulance.nunique()}`"
                )
