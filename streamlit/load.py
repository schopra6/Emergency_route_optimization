...
def add_traffic_area(stops_df: pd.DataFrame, traffic_bounds: list):
    traffic_area = box(*traffic_bounds)

    stops_df_gpd = gpd.GeoDataFrame(stops_df, geometry="geometry")

    stops_in_traffic_area = stops_df_gpd[stops_df_gpd.within(traffic_area)]

    return stops_in_traffic_area, traffic_area
