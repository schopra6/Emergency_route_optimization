...
def define_model(
    ...
    **params,
    ...
    model.addConstrs(
        (
            u[j] - u[i] >= demand[j] - AMBULANCE_CAPACITY * (1 - x[i, j, k])
            for i in stops[1:]
            for j in stops[1:]
            for k in range(buses)
            if i != j
    ...
    model.addConstrs(
        (u[i] <= AMBULANCE_CAPACITY for i in stops[1:]),
...
def solve_model(
    ...
    distance_matrix_model = distance_matrix.drop(
        columns=stops_in_traffic_area.stop_id, index=stops_in_traffic_area.stop_id
    )
    ...
    st.markdown(f"`Total capacity: {num_buses * AMBULANCE_CAPACITY}`")

    if sum(demand.values()) > num_buses * AMBULANCE_CAPACITY:
        st.error("⛔ Total demand exceeds total capacity")

    new_demand, nodes_exceeding_demand = split.reconstruct_demand(
        demand, AMBULANCE_CAPACITY, rng, split_type
    )
